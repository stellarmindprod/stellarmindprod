...
def manage_timetable_page():
    ...
    try:
        ...
        if selected_semester:
            ...
            params_tt = {
                'select': 'id,day_of_week,start_time,end_time,subject_code,teacher_name,courses(course_name)',
...
class TimetableEntry(BaseModel):
    ...
    subject_code: str = Field(description="Subject code or name. Can be empty if it is a break or free period.")
    teacher_name: str = Field(description="Name of the teacher taking the class. Can be empty.")
...
def upload_timetable_excel():
    ...
    if file and (file.filename.endswith('.xlsx') or file.filename.endswith('.xls')):
        try:
            ...
            # 2. Setup LLM Parser
            parser = JsonOutputParser(pydantic_object=TimetableEntry)
            prompt = PromptTemplate(
                template="You are a data extraction assistant. Follow the format instructions and extract the timetable schedule from the given unstructured table into a structured JSON array of entries. Provide exactly a JSON array according to the schema. Make sure to assign semester to {semester} for all records. \n\n{format_instructions}\n\n{table}",
                input_variables=["semester", "table"],
                partial_variables={"format_instructions": parser.get_format_instructions()},
            )

            # 3. Use LLM - Ensure HUGGINGFACEHUB_API_TOKEN is set in environment
            os.environ["HUGGINGFACEHUB_API_TOKEN"] = "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"
            hf_token = os.environ.get("HUGGINGFACEHUB_API_TOKEN")
            if not hf_token:
                flash("HUGGINGFACEHUB_API_TOKEN environment variable not set. Please set it in config.py or environment to use AI upload.", "danger")
                return redirect(url_for('manage_timetable_page', semester=semester))
            
            repo_id = "meta-llama/Meta-Llama-3-8B-Instruct" 
            llm = HuggingFaceEndpoint(repo_id=repo_id, task="text-generation", temperature=0.1, max_new_tokens=4000)
            chain = prompt | llm | parser
            
            structured_entries = chain.invoke({"semester": semester, "table": markdown_content})
            ...
            for entry in structured_entries:
                new_entry = {
                    ...
                    "subject_code": entry.get('subject_code') or None,
                    "teacher_name": entry.get('teacher_name') or None
                }
                url = get_supabase_rest_url(TIMETABLE_TABLE)
                headers = SUPABASE_HEADERS.copy()
                headers['Prefer'] = 'return=minimal'
            ...
            flash(f"Successfully processed and added {success_count} timetable entries via AI!", "success")
        except Exception as e:
            print(f"Error parsing or uploading Excel: {e}")
            flash(f"Error processing Excel using AI: {str(e)[:100]}", "danger")
    else:
